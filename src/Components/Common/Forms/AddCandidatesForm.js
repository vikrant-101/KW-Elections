                ...
                return (
                  <Col key={items?.id} sm={6}>
                    <div className="mb-3">
                      ...
                      <div className="lang-change-wrap">
                        <Input type="number" className="form-control" placeholder={items.labelName} defaultValue={formData[items?.value]} name={items?.name} disabled={items?.name === 'LabelKey' && isAddOrEdit === 'isEdit' ? true : false} onChange={(e) => onChangeHandler(e)} id={items?.labelName} required />
                      </div>
                    </div>
                  </Col>
                )
