...
const AreaWiseReport = () => {
  ...
  document.title = t('KW-Elections | Area Wise Report');
  const dispatch = useDispatch();


  const { AreaWise, isLoading, columnNames, printDetail } = useSelector((state) => {
    return {
      AreaWise: state.AnalyticalReports.areaWise,
      columnNames: state.AnalyticalReports.areaWiseColumnNames,
      isLoading: state.AnalyticalReports.isLoading,
      printDetail: state.Voters.printDetail[0]
    }
  });

  const [data, setData] = useState(AreaWise)
  ...
  useEffect(() => {
    dispatch(getAreaWiseReport({ userID: user.id }))
    dispatch(getAreaWiseTableColumnNames())
  ...
  const printTable = (e, columns) => {
    ...
    newWindow.document.write(`<style>title { text-align: right; }</style><title>Area Name Wise List</title>`);
    ...
    if (printDetail?.CandidateName != undefined && printDetail?.ElectionName != undefined) {
      ...
      <div>
        <h3>قائمة الحكمة لاسم المنطقة</h3>
      </div>
      <div>
        ...
        <Container fluid>
          ...
          <Row className='mb-3'>
            ...
            <Col className="col-12 col-md-3 col-lg-3">
              <label> &nbsp;</label> <br />
              <SearchTextBox initialData={AreaWise} setData={setData} />
            </Col>
          </Row>
          <Row>
            <Col>
