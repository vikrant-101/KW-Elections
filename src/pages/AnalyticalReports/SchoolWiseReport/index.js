...
import { getSchoolWiseReport, getSchoolWiseTableColumnNames } from "../../../store/analyticalReport/actions";
...
const SchoolWiseReport = () => {
  ...
  document.title = t('KW-Elections | School Wise Report');
  const dispatch = useDispatch();

  const { SchoolWise, isLoading, columnNames, printDetail } = useSelector((state) => {
    return {
      SchoolWise: state.AnalyticalReports.schoolWise,
      columnNames: state.AnalyticalReports.schoolWiseColumnNames,
      isLoading: state.AnalyticalReports.isLoading,
      printDetail: state.Voters.printDetail[0]
    }
  });

  const [data, setData] = useState(SchoolWise)
  ...
  const printTable = (e, columns) => {
    ...
    if (printDetail?.CandidateName != undefined && printDetail?.ElectionName != undefined) {
      ...
      <div>
        <h3>قائمة اسم المدرسة الحكيمة</h3>
      </div>
      <div>
        ...
        <Container fluid>
          ...
          <Row className='mb-3'>
            ...
            <Col className="col-12 col-md-3 col-lg-3">
              <label> &nbsp;</label> <br />
              <SearchTextBox initialData={SchoolWise} setData={setData} />
            </Col>
          </Row>
          <Row>
            <Col>
