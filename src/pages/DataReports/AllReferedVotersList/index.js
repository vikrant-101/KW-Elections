...
import { getAllReferVotersList, getAllReferVotersListTableColumnNames } from "../../../store/allReferVotersList/actions";
...
const AllReferedVotersList = () => {
  ...
  document.title = t('KW-Elections | All Refered Voters');
  const dispatch = useDispatch();
  const { AllReferedVotersList, isLoading, columnNames, printDetail } = useSelector((state) => {
    return {
      AllReferedVotersList: state.AllReferedVotersList.allrefervoterslist,
      columnNames: state.AllReferedVotersList.columnNames,
      isLoading: state.AllReferedVotersList.isLoading,
  ...
  useEffect(() => {
    dispatch(getAllReferVotersList({ userID: user.id }))
    dispatch(getAllReferVotersListTableColumnNames())
  ...
  const printTable = (e, columns) => {
    ...
    newWindow.document.write(`<style>title { text-align: center; }</style><title>Refer Voter List</title>`);
    ...
    if (printDetail?.CandidateName != undefined && printDetail?.ElectionName != undefined) {
      ...
      <div>
      <h2>جميع إحالة قائمة الناخبين</h2>
      </div>
      <div>
  ...
  newWindow.document.write(`<style>
    ...
    AllReferedVotersList?.forEach((row) => {
          ...
          <Row className='mb-3'>
            <Col className="col-12 col-md-3 col-lg-4">
              <div className="mb-3 cis-manage-demo-btn">
                <Link to="#" onClick={(e) => { printTable(e, columns(columnNames, i18n, t)) }} className="btn btn-success mr-15px" style={{
                  margin: "5px"
                }}>
            ...
            <Col className="col-12 col-md-3 col-lg-3">
              <label> &nbsp;</label> <br />
              <SearchTextBox initialData={AllReferedVotersList} setData={setData} />
            </Col>
          </Row>
          <Row>
            <Col>
